            ...
            <table class="table" id="paginationNumbers" class="display nowrap">
                ...
                <tr>
                    ...
                    <td>{{$offer->offer_date->format('d.m.Y')}}</td>
                        ...
                        @if($offer->product==1 && $offer->accept_type=='Aylık')
                         <td>{{number_format($offer->offer_money,2,',','.')}}</td>
                            @elseif($offer->product==1)
                                <td>%{{$offer->offer_money}}</td>
                            @elseif($offer->product==6)
                            <td>{{$offer->offer_total}}</td>
                    @else
                        <td>{{number_format($offer->offer_total,0,',','.').' '}}TL</td>
                    @endif

                    <td>{{$offer->tproduct->name}}</td>

                    <td>{{!empty($offer->seller->seller_name) ? $offer->seller->seller_name : null }}</td>
                    <td > <textarea>@foreach($offer->explanations as $explanation)
                            {{$explanation->date->format('d.m.Y').''.$explanation->explanation}} @endforeach </textarea>
                    </td>
                    <td>
                        <a href="{{route('customer.edit',['id'=>$offer->customer_id])}}" ><button class="btn btn-success btn-xs">Müşteri Düzenle</button></a>
                    </td>

                    <td>@if($offer->offer_status==1)
                            <a href="{{route('offer.status',['id'=>$offer->id])}}" ><button class="btn btn-warning btn-xs">Bekleniyor</button></a>

                        @elseif($offer->offer_status==2)
                            <a href="{{route('offer.status',['id'=>$offer->id])}}" ><button class="btn btn-success btn-xs">Kabul Edildi</button></a>
                    ...
                    <td><a href="{{route('offer.edit',['id'=>$offer->id])}}"><button class="btn btn-primary btn-xs">Güncelle</button></a>
                        <a href="{{route('contract_upload',['id'=>$offer->id])}}"><button class="btn btn-primary btn-xs">Sözleşme Yükle</button></a>

                        <a href="{{route('offer.delete',['id'=>$offer->id])}}"><Button class="btn btn-danger ">Sil</Button></a>
                        <a href="{{route('offer.file',['id' => $offer->id])}}"><img src="https://img.icons8.com/officel/40/000000/pdf.png"/></a>
                       <!-- <a href="/pdf/">-->
                    </td>
            ...
            <div class="col-md-2 col-sm-3 col-xs-12">
                <div class="info-box bg-green">
                    ...
                    <div class="info-box-content">
                        ...
                        <span class="info-box-number">{{number_format($offers->whereNotIn('product',1)->where('offer_status',2)->whereNotIn('product',1)->sum('offer_total'),2,',','.')}}</span>
                    </div>
                    <!-- /.info-box-content -->
                </div>
                <!-- /.info-box -->
            ...
            <div class="col-md-2 col-sm-3 col-xs-12">
                <div class="info-box bg-yellow">
                    ...
                    <div class="info-box-content">
                        ...
                        <span class="info-box-number">{{number_format($offers->whereNotIn('product',1)->sum('offer_total'),2,',','.')}}</span>
                    </div>
                    <!-- /.info-box-content -->
